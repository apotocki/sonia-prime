...
class utf16_to_utf16be
    : public converter_facade<
          utf16_to_utf16be<ProviderT, CustomerT, ErrorHandlerT>
        , CustomerT
        , ProviderT
        , char16_t
        , char
...
{
protected:
    typedef typename utf16_to_utf16be::facade_type base_type;
...
private:
    ...
    inline static void do_clear(utf16_tag<HT> const&, utf16be_tag<ErrorHandlerT> const& t, StateT& st) {
        st.clear(t.errh_);
    }
    ...
    static bool do_get(ForwardReadableIterator& in, StateT& s, char * result)
    {
        ...
        for (;;) {
            ...
            if (tmp_result < UTF16_SUR_HIGH_START || tmp_result > UTF16_SUR_LOW_END) {
                st.cached = 1;
                st.value = tmp_result & 0x00FF;
                *result = static_cast<char>(tmp_result >> 8);
                return true;
                ...
                char32_t trail = ov & 0xFFFF;
                if (trail >= UTF16_SUR_LOW_START || trail <= UTF16_SUR_LOW_END) {
                    st.cached = 3;
                    st.value = ((tmp_result << 16) & 0x00FF0000) | trail;
                    *result = static_cast<char>(tmp_result >> 8);
                    return true;
                } else {
