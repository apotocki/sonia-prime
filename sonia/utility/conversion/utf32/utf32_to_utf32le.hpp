...
class utf32_to_utf32le
    : public converter_facade<
          utf32_to_utf32le<ProviderT, CustomerT, ErrorHandlerT>
        , CustomerT
        , ProviderT
        , char32_t
        , char
...
{
protected:
    typedef typename utf32_to_utf32le::facade_type base_type;
...
private:
    ...
    inline static void do_clear(utf32_tag<HT> const&, utf32le_tag<ErrorHandlerT> const& t, StateT & st) {
        st.clear(t.errh_);
    }
    ...
    static bool do_get(ForwardReadableIterator& in, StateT& s, char * result) 
    {
        ...
        for (;;) {
            if (st.cached) {
                --st.cached;
                *result = static_cast<char>(st.value >> (16 - 8 * st.cached));
                return true;
            }
            char32_t ov;
