...
struct expression_cast_to_object_visitor : static_visitor<optional<beng_type>>
{
    fn_compiler_context& ctx;
    beng_object_t const& target;
    std::vector<semantic_expression_type>& result;

    expression_cast_to_object_visitor(fn_compiler_context& c, beng_object_t const& t, std::vector<semantic_expression_type>& r)
        : ctx{ c }
        , target{ t }
        , result{ r }
    {}
    ...
    */
    ...
    inline result_type operator()(beng_object_t const& v) const
    {
        ...
        if (auto const* pte = dynamic_cast<type_entity const*>(v.value); pte) {
            if (pte->try_cast(ctx, result, target)) {
                return target;
            }
        }
        return nullopt;
