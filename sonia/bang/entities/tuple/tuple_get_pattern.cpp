...
{
    identifier slfid = ctx.u().get_self_parameter_identifier();
    identifier propid = ctx.u().get_property_parameter_identifier();

    tuple_entity const* pte = nullptr;
    identifier_entity const* ppname = nullptr;
    ...
    for (auto const& arg : call.args()) {
        ...
        if (pargname->value == slfid && !pte) {
            pmd = make_shared<functional_match_descriptor>(ctx.u());
            auto last_expr_it = ctx.expressions().last();
            expression_visitor evis{ ctx };
            auto res = apply_visitor(evis, arg.value());
            if (!res) return std::unexpected(std::move(res.error()));
            if (ctx.context_type) {
                entity const& some_entity = ctx.u().eregistry().get(ctx.context_type);
                pte = dynamic_cast<tuple_entity const*>(&some_entity);
                if (pte) {
    ...
    BOOST_ASSERT(pes);
    size_t index;
    field_descriptor const* fd = pes->find_field(ppname->value(), &index);
    if (!fd) {
        return std::unexpected(make_error<basic_general_error>(call.location(), "no such field"sv, ppname->value()));
    }
...
error_storage tuple_get_pattern::apply(fn_compiler_context& ctx, qname_identifier functional_id, functional_match_descriptor& md) const
{
    ...
    identifier propid = u.get_property_parameter_identifier();
    auto optsp = md.bindings.lookup(u.get_property_parameter_identifier());
    if (optsp) {
        ...
        u.push_back_expression(args, semantic::invoke_function(u.get_array_at_entity_identifier()));
    }
