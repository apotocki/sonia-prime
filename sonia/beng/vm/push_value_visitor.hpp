...
public:
    ...
    void operator()(lang::beng::function_value const& dval) const
    {
        if (auto eptr = unit_.eregistry().find(dval.mangled_name); eptr) {
            if (auto fe = dynamic_pointer_cast<lang::beng::function_entity>(eptr); fe) {
                if (!fe->is_defined()) {
                ...
                if (!fe->captured_variables.empty()) {
                    ...
                    if (fe->captured_variables.size() > 1) {
                        ...
                        //bvm().push_on_stack_and_push(ui64_blob_result(fe->captured_variables.size()));
                        //bvm().append_builtin(sonia::lang::beng::builtin_type::arrayify);
                    }
                } else {
