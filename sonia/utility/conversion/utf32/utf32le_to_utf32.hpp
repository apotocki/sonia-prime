...
class utf32le_to_utf32
    : public converter_facade<
          utf32le_to_utf32<ProviderT, CustomerT, ErrorHandlerT>
        , CustomerT
        , ProviderT
        , char
        , char32_t
...
{
protected:
    typedef typename utf32le_to_utf32::facade_type base_type;
...
private:
    ...
    inline static void do_clear(utf32le_tag<HT> const&, utf32_tag<ErrorHandlerT> const& t, StateT & st) {
        st.clear(t.errh_);
    }
    ...
    static void do_put(char val, ForwardWritableIterator& out, StateT& s)
    {
        ...
        if (st.cached != 3) {
            st.value |= (static_cast<char32_t>(val)&0xff) << (8*st.cached);
            st.cached++;
            return;
        }
        char32_t result = st.value | (val & 0x000000ff) << 24;
        st.reset();
