...
#include "sonia/fibers/detail/futex.hpp"
...
public:
    ...
    void lock() noexcept {
        ...
        const std::int32_t max_relax_retries = (std::min)(
                static_cast< std::int32_t >( SONIA_FIBERS_SPIN_BEFORE_SLEEP0), 2 * prev_retries + 10);
        const std::int32_t max_sleep_retries = (std::min)(
                static_cast< std::int32_t >( SONIA_FIBERS_SPIN_BEFORE_YIELD), 2 * prev_retries + 10);
        // after max. spins or collisions suspend via futex
        while ( retries++ < SONIA_FIBERS_RETRY_THRESHOLD) {
            ...
            if ( 0 != ( expected = value_.load( std::memory_order_relaxed) ) ) {
#if !defined(SONIA_FIBERS_SPIN_SINGLE_CORE)
                ...
                std::uniform_int_distribution< std::int32_t > distribution{
                    0, static_cast< std::int32_t >( 1) << (std::min)(collisions, static_cast< std::int32_t >( SONIA_FIBERS_CONTENTION_WINDOW_THRESHOLD)) };
