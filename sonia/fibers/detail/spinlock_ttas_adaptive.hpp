    ...
    void lock() noexcept {
        ...
        for (;;) {
            ...
            const std::size_t max_relax_retries = (std::min)(
                    static_cast< std::size_t >( SONIA_FIBERS_SPIN_BEFORE_SLEEP0), 2 * prev_retries + 10);
            const std::size_t max_sleep_retries = (std::min)(
                    static_cast< std::size_t >( SONIA_FIBERS_SPIN_BEFORE_YIELD), 2 * prev_retries + 10);
            ...
            if ( spinlock_status::locked == state_.exchange( spinlock_status::locked, std::memory_order_acquire) ) {
                ...
                std::uniform_int_distribution< std::size_t > distribution{
                    0, static_cast< std::size_t >( 1) << (std::min)(collisions, static_cast< std::size_t >( SONIA_FIBERS_CONTENTION_WINDOW_THRESHOLD)) };
